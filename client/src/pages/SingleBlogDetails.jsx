...
const SingleBlogDetails = () => {
  ...
  const { data, loading, error } = useFetch(
    `${getEnv("VITE_API_BASE_URL")}/blog/getBlog/${blog}`,
    {
      method: "get",
      credentials: "include",
    },[blog, category]
    ...
    <div className=" md:flex-nowrap flex-wrap flex justify-between gap-20">
      {data && data.blog && (
        <div className="border rounded md:w-[70%] w-full p-5">
          ...
          ></div>
          <div className="border-t mt-5 pt-5"><Comment props = {{blogId: data.blog._id}}/></div>
          {/* <div className="border-t mt-5 pt-5"><CommentList props = {{blogId: data.blog._id}}/></div> */}
        </div>
      )}
